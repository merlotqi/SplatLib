...
std::unique_ptr<splat::DataTable> readInput(const std::filesystem::path& path) {
    if (!path.has_extension())
        throw std::runtime_error("Input file has no extension");
    std::string ext = path.extension().string();
    std::transform(ext.begin(), ext.end(), ext.begin(), [](unsigned char c){ return std::tolower(c); });
    if (ext == ".ply")
        return splat::readPly(path.string());
    if (ext == ".splat")
        return splat::readSplat(path.string());
    if (ext == ".sog")
        return splat::readSog(path.string(), path.string());
    if (ext == ".spz")
        return splat::readSpz(path.string());
    if (ext == ".ksplat")
        return splat::readKsplat(path.string());
...
// RAII for GLFW
struct GlfwSession {
    GlfwSession() { if (!glfwInit()) throw std::runtime_error("Failed to initialize GLFW"); }
...
int main(int argc, char** argv) {
    try {
        ...
        std::unique_ptr<GLFWwindow, decltype(&glfwDestroyWindow)> window(
            glfwCreateWindow(options.width, options.height, "PlaycanvasViewer", nullptr, nullptr),
            glfwDestroyWindow);
        if (!window)

...
void test_decodePlayCanvasColor() {
    assert(std::abs(decodePlayCanvasColor(0) - 0.5f) < 1e-6f);
}

void test_decodePlayCanvasOpacity() {
    assert(std::abs(decodePlayCanvasOpacity(0) - 0.5f) < 1e-6f);
}

void test_decodePlayCanvasScale() {
    assert(std::abs(decodePlayCanvasScale(std::log(5.0f)) - 5.0f) < 1e-5f);
}
...
void test_adaptDataTableToGSplat_maxSplats() {
    ...
    std::vector<std::string> names = {"x","y","z","rot_0","rot_1","rot_2","rot_3","scale_0","scale_1","scale_2","f_dc_0","f_dc_1","f_dc_2","opacity"};
